 ...
 * @LastEditTime: 2022-04-24 09:34:33
...
class ModalListPage extends React.Component {
    ...
    changeKeyGeneral = async (item, modalData) => {
        ...
        } else if (item.changeKey == 'paperPoint') {
        } else {
            // 获取当前修改的key
     ...
     * 树形结构选中的数据
     * @param {当前操作的数据} item
     * @param {当前选中的key} value
     * @param {选中的全部数据} checkData
     */
    treeDataChange = async (item, value, checkData) => {
        let modalData = this.props.modalData, // 获取弹窗已经填写的数据
            checkedNodes = checkData.checkedNodes || [], // 获取选中的原数据
            checkKeys = []; // 声明选择数据字段
        // 循环选中的原数据
        for (let i = 0; i < checkedNodes.length; i++) {
            let checkItem = checkedNodes[i];
            if (!checkItem.children) {
                checkKeys.push(checkItem.questionCategoryId);
            }
        }
        modalData[item.key] = checkKeys; // 给当前项的key赋值
        // 判断当前项是否存在修改其他的可以、值
        if (item.changeKey) {
            modalData = await this.changeKeyGeneral(item, modalData);
        }
     ...
     * 上传文件
     * @param {当前操作的数据} item
     * @param {上传的类型} uploadType
     * @param {上传的文件} param2
     */
    uploadChange = (item, uploadType, { file }) => {
        // 获取全部的填写数据
        let modalData = this.props.modalData;
        // 赋值一个上传类型
        modalData.uploadType = uploadType;
        // 对当前项进行赋值
        if (item.maxCount && item.maxCount > 1) {
        } else {
            modalData[item.key] = [file];
        }
        this.props.changeModalData(modalData, item);
    };
    ...
    inputQuestionTypeChange = (item, itemChild, changeKey, val) => {
        ...
        // 判断是否需要修改其他的key值
        if (item.changeKey) {
            modalData[item.changeKey] = this.questionTypeChangeKey(
                item,
                itemData,
            );
        }
    ...
    questionTypeChangeKey = (item, itemData) => {
        ...
        for (let i = 0; i < childList.length; i++) {
            ...
            if (typesNumber[childItem.value] && typesPoint[childItem.value]) {
                totalPoint +=
                    typesNumber[childItem.value] * typesPoint[childItem.value];
            }
        }
        return totalPoint;
    };
        ...
        if (item.inputType == 'number') {
            return (
                <InputNumber
                    ...
                    value={this.props.modalData[item.key]}
                    ...
                    onChange={this.inputChange.bind(this, item, 'number')}
                ></InputNumber>
            );
            // 判断返回textarea组件
            ...
            return (
                <Input.TextArea
                    ...
                    disabled={item.disabled || false}
                    placeholder={item.placeholder}
                    onChange={this.inputChange.bind(this, item, 'other')}
                ></Input.TextArea>
            );
            // 其他的全部返回输入框组件
        } else {
    ...
    renderSelectHtml = (item) => {
        ...
        return (
            <Select
                ...
                value={this.props.modalData[item.key]}
                placeholder={item.placeholder}
                onChange={this.generalDataChange.bind(this, item)}
            ></Select>
        );
    };
        ...
        switch (item.timeType) {
            case 'timer':
                return (
                    <TimePicker
                        allowClear
                        onChange={this.datePickerSectionChange.bind(this, item)}
                        disabled={item.disabled}
                        value={itemValue}
                    ></TimePicker>
                );
    ...
    renderSwitchHtml = (item) => {
        return (
            <Switch
                ...
                onChange={this.generalDataChange.bind(this, item)}
                disabled={item.disabled}
                checked={this.props.modalData[item.key]}
            />
        );
    };
    ...
    renderTreeHtml = (item) => {
        ...
        return (
            <Tree
                checkable
                treeData={childList}
                checkedKeys={this.props.modalData[item.key] || []}
                fieldNames={fieldNames}
                onCheck={this.treeDataChange.bind(this, item)}
                disabled={item.disabled}
            />
        );
    };
    ...
    renderRadioHtml = (item) => {
        ...
        return (
            <Radio.Group
                disabled={item.disabled}
                value={this.props.modalData[item.key]}
                onChange={this.generalDomChange.bind(this, item, 'radio')}
            >
                {childList.map((itemChild, ind) => {
                    return (
                        <Radio key={ind} value={itemChild.value}>
                            {itemChild.label}
                        </Radio>
                    );
                })}
    ...
    renderCheckBoxHtml = (item) => {
        ...
        return (
            <Checkbox.Group
                ...
                options={childList}
                value={this.props.modalData[item.key] || []}
                onChange={this.generalDomChange.bind(this, item, 'checkbox')}
            ></Checkbox.Group>
        );
    };

    // 上传文件
    renderUploadHtml = (item) => {
        let fileType = item.fileType || 'image/*',
            fileBtnList = item.fileBtnList || [],
            fileList = this.props.modalData[item.key] || [];
        return (
            <div className="uploadArea">
                {fileBtnList.map((btnItem, ind) => {
                    let maxCount = btnItem.maxCount || 1;
                    return (
                        <Upload
                            key={ind}
                            maxCount={maxCount}
                            ...
                            disabled={item.disabled}
                            accept={btnItem.fileType}
                            beforeUpload={() => {
                                return false;
                            }}
                        >
                            {fileList.length < maxCount ? (
                                <Button className="uploadBtn" type="primary">
                                    {btnItem.btnName}
                                </Button>
                            ) : (
                                ''
    ...
    renderQuestionTypesNumberHtml = (item) => {
        ...
        return (
            <div className="questionTypeNumber">
                {childList.map((itemChild, ind) => {
                    return (
                        <div key={ind} className="questionTypeNumberItem">
                            {itemChild.name}：{typeNumber[itemChild.value] || 0}
                            道
                        </div>
                    );
                })}
    ...
    renderQuestionTypesHtml = (item) => {
        ...
        return (
            <div className="questionTypes">
                {childList.map((itemChild, ind) => {
                    return (
                        <div key={ind} className="questionTypesItem">
                            <div className="questionTypesItemText">
                                {itemChild.name}：
                            </div>
                            <InputNumber
                                className="questionTypesItemInput"
                                type="number"
                                addonAfter="道"
                                value={typesNumber[itemChild.value]}
                                disabled={item.disabled || false}
                                placeholder="请输入数量"
                                onChange={this.inputQuestionTypeChange.bind(
                                    this,
                                    item,
                                    itemChild,
                                    'typesNumber',
                            ...
                            ></InputNumber>
                            <div className="questionTypesItemText questionTypesItemTextPoint">
                                每道题：
                            </div>
                            <InputNumber
                                className="questionTypesItemInput"
                                type="number"
                                addonAfter="分"
                                value={typesPoint[itemChild.value]}
                                disabled={item.disabled || false}
                                placeholder="请输入分数"
                                onChange={this.inputQuestionTypeChange.bind(
                                    this,
        ...
        return (
            <>
                {childList.map((childItem) => {
                    ...
                    return (
                        <div className="modalRadioList" key={childItem.value}>
                            <div className="modalRadioListItem">
                                {childItem.label}：
                            </div>
                            <Radio.Group
                                disabled={item.disabled}
                                value={itemData[childItem.value]}
                                onChange={this.radioListChange.bind(
                                    this,
                                    item,
                                    childItem.value,
                                )}
    ...
    renderInputListHtml = (item) => {
        ...
        if (itemData.length <= 0) itemData = [''];
        return (
            <div className="modalInputList">
                <div className="modalInputLeft">
                    {itemData.map((itemChild, ind) => {
                        return (
                            <Input
                                className="modalInputListInput"
                                key={ind}
                                value={itemChild}
                                suffix={
                                    <MinusCircleFilled
                                        onClick={this.addDeleteInputListData.bind(
                                            this,
                                            item,
                                            'delete',
                                            ind,
                                ...
                                disabled={item.disabled}
                                placeholder={item.placeholder}
                                onChange={this.inputListChange.bind(
                                    this,
                                    item,
                                    ind,
                                )}
    ...
    renderOptionInputListHtml = (item) => {
        let itemData = this.props.modalData[item.key] || {},
            childList = this.state.questionOptionList;
        return (
            <div className="modalInputList modalOptionInputList">
                <div className="modalInputLeft">
                    {childList.map((itemChild, ind) => {
                        return (
                            <div key={ind} className="modalInputLeftItem">
                                {itemChild.label}：
                                <Input
                                    className="modalOptionInputListInput"
                                    key={ind}
                                    value={itemData[itemChild.value]}
                                    suffix={
                                        <MinusCircleFilled
                                            onClick={this.addDeleteOptionInputListData.bind(
                                                this,
                                                item,
                                                'delete',
                                                ind,
                                    ...
                                    disabled={item.disabled}
                                    placeholder={item.placeholder}
                                    onChange={this.optionInputListChange.bind(
                                        this,
                                        item,
                                        itemChild.value,
                                    )}
    ...
    renderModalRightHtml = (item) => {
        switch (item.type) {
            ...
            case 'upload':
                return this.renderUploadHtml(item);
            default:
                return this.renderInputHtml(item);
        }
    };

    render() {
        return (
            <div className="modalListPage">
                {this.props.modalList.map((item, index) => {
                    if (item.type == 'title') {
                        return (
                            <div
                                key={index}
                                className="modalListItem modalListItemTitle"
                            >
                                {item.name}
                            </div>
                        );
                    } else {
                        return judgeItemShow(item, this.props.modalData) ? (
                            <div key={index} className="modalListItem">
                                <div className="modalListLeft">
                                    {item.required ? (
                                        <div className="modalRequired">*</div>
                                    ) : (
                                        ''
                                    )}
                                    {item.name}：
                                ...
                                <div className="modalListRight">
                                    {this.renderModalRightHtml(item)}
                                </div>
                            </div>
                        ) : (
                            ''
