...
class CommonModal extends React.Component{
        ...
        if(modalList.length > 0){
            ...
            for(let i = 0; i < copyModalList.length; i++){
                ...
                if(modalItem.reqDataUrl && !modalItem.reqDataKey){
                    // 请求子元素数据
                    let childList = await this.requestListData(modalItem.reqDataUrl);
                    // 如果当前是select数据 则进行特殊处理
                    if(['select', 'selectPaper'].includes(modalItem.type)){
                        childList = this.disposeSelectData(childList, modalItem)
                    }
    ...
    disposeSelectData = (childList, item) => {
        ...
        for(let i = 0; i < childList.length; i++){
            // 获取当前循环项数据           声明新的当前循环项
            let childItem = childList[i], newChildItem = {};
            // 对当前循环项进行格式化显示
    ...
    linkageChild = async(item) => {
        ...
        // 判断当前项需要的请求参数是否存在
        if(itemKey){
            childList = await this.requestListData(linkageItem.reqDataUrl + itemKey);
            // 如果当前是select数据 则进行特殊处理
    ...
    clickOk = () => {
        ...
        for(let i = 0; i < copyModalList.length; i++){
            ...
            // 判断当前项是否必填并且不是为空
            if(modalItem.required && !itemKeyData){
    ...
    render(){
        return <Modal width={this.props.popWidth} className="commonModal" destroyOnClose={true} centered={true} maskClosable={false} footer={this.props.footer}
            visible={true} onOk={this.clickOk} onCancel={this.props.cancelCallBack} confirmLoading={this.props.okLoading}
            title={this.props.title} cancelText={this.props.cancelText} okText={this.props.okText}>
                <Spin spinning={this.state.spinLoading}>
                    {this.props.modalList.length > 0 ? <ModalListPage modalList={this.state.copyModalList} modalData={this.state.modalData} changeModalData={this.changeModalData}></ModalListPage> : this.props.children}
                </Spin>
            </Modal>
    }
}
